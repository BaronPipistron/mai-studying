...
void create_helper(void* parent_socket, void* socket, int& create_id, int& id,
                   int& pid) 
{
   if (pid == -1) {
      send_message(parent_socket, "CREATE_HELPER ERROR: Cannot fork");
      pid = 0;
   } else if (pid == 0) {
...
{
   if (pid == 0) {
      std::string receive_message = "EXEC_HELPER ERROR:" + std::to_string(id);
      ...
      if (str == "") str = "EXEC_HELPER ERROR: Node is unavailable";
...
{
   if (pid == 0) {
      std::string receive_message = "PING_HELPER ERROR:" + std::to_string(id);
      receive_message += ": Not found";
      send_message(parent_socket, receive_message);
   } else {
      send_message(socket, request_string);
          ...
          int& id, std::string& request_string) 
{
   ...
   if (exec_id == id) {
      command_stream >> nums_string;
      std::string reply = "";
      int index = 0;

      std::string substr_num = "";
      for (size_t i = 0; i != nums_string.length(); ++i) {
        if (nums_string[i] != '_') {
            substr_num += nums_string[i];
        } else {
            sum += std::stoi(substr_num);
            substr_num = "";
        }
      }

      sum += std::stoi(substr_num);
      reply = "Ok:" + std::to_string(id) + ":" + std::to_string(sum);
      send_message(parent_socket, reply);
   } else if (exec_id < id) {
      exec_helper(parent_socket, left_socket, exec_id, left_pid,
                  request_string);
   } else {
      exec_helper(parent_socket, right_socket, exec_id, right_pid,
                  request_string);
   }
}
          ...
          int& id, std::string& request_string) 
{
   ...
   if (ping_id == id) {
      ...
      ping_helper(parent_socket, left_socket, ping_id, left_pid,
                  request_string);
   } else {
      ping_helper(parent_socket, right_socket, ping_id, right_pid,
                  request_string);
   }
}
   ...
   while (true) {
      ...
      if (command == "create") {
         ...
         if (create_id == id) {
            ...
            if (left_pid == 0) {
               zmq_bind(left_socket,
                        get_port_name(DEFAULT_PORT + create_id).c_str());
                        
               zmq_setsockopt(left_socket, ZMQ_RCVTIMEO, NULL, nl * 500);
               zmq_setsockopt(left_socket, ZMQ_SNDTIMEO, NULL, nl * 500);
               left_pid = fork();
               create_helper(parent_socket, left_socket, create_id, left_id,
                             left_pid);
            } else {
               ...
               if (reply == "") {
                  reply = "Error: Node is unavaliable";
               } else {
                  zmq_setsockopt(left_socket, ZMQ_RCVTIMEO, NULL, nl * 500);
                  zmq_setsockopt(left_socket, ZMQ_SNDTIMEO, NULL, nl * 500);
               }
      ...
      if (command == "kill") {
         ...
         if (delete_id < id) {
            kill_node(parent_socket, left_socket, delete_id, left_id, left_pid,
                      request);
         } else {
            kill_node(parent_socket, right_socket, delete_id, right_id,
                      right_pid, request);
         }
      }
